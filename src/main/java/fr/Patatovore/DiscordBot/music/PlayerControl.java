package fr.Patatovore.DiscordBot.music;
...
public class PlayerControl extends ListenerAdapter
{
    ...
    public void onMessageReceived(MessageReceivedEvent event)
    {
        ...
        if ("!join".equals(command[0]))
        {
            ...
            {
                event.getChannel().sendMessage("Veuillez spécifier un salon à rejoindre.").queue();
            }
            else
            {
                ...
                if (chan == null)
                {
                    event.getChannel().sendMessage("Impossible de trouver le channel vocal : " + command[1]).queue();
                }
                else
                {
                    ...
                    catch (PermissionException e)
                    {
                        ...
                        {
                            event.getChannel().sendMessage("AmplifiaBot n'a pas la permission de se connecter à : " + chan.getName()).queue();
                        }
                    }
                }
            }
        ...
        else if ("!play".equals(command[0]))
        {
            if (command.length == 1) //It is only the command to start playback (probably after pause)
            {
                if (player.isPaused())
                {
                    ...
                    event.getChannel().sendMessage("La lecture a été reprise").queue();
                }
                else if (player.getPlayingTrack() != null)
                {
                    event.getChannel().sendMessage("La musique est déjà en train d'être jouée!").queue();
                }
                else if (scheduler.queue.isEmpty())
                {
                    event.getChannel().sendMessage("La PlayList est vide, ajoutez quelque-chose en premier!").queue();
                }
            }
        ...
        {
            scheduler.nextTrack();
            event.getChannel().sendMessage("La musique a été passée.").queue();
        }
        else if ("!pause".equals(command[0]))
        {
            if (player.getPlayingTrack() == null)
            {
                event.getChannel().sendMessage("Impossible de mettre sur pause puisqu'aucune musique n'est jouée.").queue();
                return;
            }

            player.setPaused(!player.isPaused());
            if (player.isPaused())
                event.getChannel().sendMessage("la musique a été mise sur pause.").queue();
            else
                event.getChannel().sendMessage("La musique a été réactivée.").queue();
        }
        else if ("!stop".equals(command[0]))
        {
            ...
            player.stopTrack();
            player.setPaused(false);
            event.getChannel().sendMessage("La musique a été stoppé et vidée.").queue();
        }
        else if ("!volume".equals(command[0]))
        {
            if (command.length == 1)
            {
                event.getChannel().sendMessage("Volume actuel: **" + player.getVolume() + "**").queue();
            }
            else
            {
                try
                {
                    ...
                    event.getChannel().sendMessage("Le volume a été changé de : `" + oldVolume + "` à : `" + newVolume + "`").queue();
                }
                catch (NumberFormatException e)
                {
                    event.getChannel().sendMessage("`" + command[1] + "` n'est pas un nombre valide. (10 - 100)").queue();
                }
            }
        }
        else if ("!restart".equals(command[0]))
        {
            ...
            if (track != null)
            {
                event.getChannel().sendMessage("Relancement de la musique: " + track.getInfo().title).queue();
                player.playTrack(track.makeClone());
            }
            else
            {
                event.getChannel().sendMessage("Aucune musique n'est jouée, il est donc impossible de la rejouer!").queue();
            }
        }
        else if ("!repeat".equals(command[0]))
        {
            scheduler.setRepeating(!scheduler.isRepeating());
            event.getChannel().sendMessage("Le lecteur a été mis en mode : **" + (scheduler.isRepeating() ? "répéter" : "ne pas répéter") + "**").queue();
        }
        else if ("!reset".equals(command[0]))
        {
            ...
            guild.getAudioManager().setSendingHandler(mng.sendHandler);
            event.getChannel().sendMessage("Le lecteur a été remis à zéro!").queue();
        ...
        else if ("!nowplaying".equals(command[0]) || ".np".equals(command[0]))
        {
            ...
            if (currentTrack != null)
            {
                ...
                String nowplaying = String.format("**Lecture de : ** %s\n**Temps::** [%s / %s]",
                        title, position, duration);

                event.getChannel().sendMessage(nowplaying).queue();
            }
            else
                event.getChannel().sendMessage("Le lecteur ne joue rien!").queue();
        }
        else if ("!list".equals(command[0]))
        {
            ...
            synchronized (queue)
            {
                if (queue.isEmpty())
                {
                    event.getChannel().sendMessage("La playlist est vide!").queue();
                }
                else
                {
                    int trackCount = 0;
                    ...
                    sb.append("Position actuelle dans la queue : ").append(queue.size()).append("\n");
                    ...
                    sb.append("\n").append("temps total de la queue: ").append(getTimestamp(queueLength));
    ...
    private void loadAndPlay(GuildMusicManager mng, final MessageChannel channel, String url, final boolean addPlaylist)
    {
        ...
        playerManager.loadItemOrdered(mng, trackUrl, new AudioLoadResultHandler()
        {
            ...
            {
                String msg = "Ajout à la liste: " + track.getInfo().title;
                if (mng.player.getPlayingTrack() == null)
                    msg += "\net le lecteur a démarré.";
